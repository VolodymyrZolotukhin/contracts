...
contract GlpAdapter is AdapterBase {
    ...
    function initialize(
        ...
        _name = string.concat(
            "VaultCraft ",
            IERC20Metadata(address(glp)).name(),
            " Adapter"
        );
        _symbol = string.concat(
            "vcGmx-",
            IERC20Metadata(address(glp)).symbol()
        );
    ...
    function _protocolWithdraw(
        ...
        //router.unstakeAndRedeemGlp(assets);
    }
    ...
    function _totalAssets() internal view override returns (uint256) {
        return IRewardTracker(router.stakedGlpTracker()).stakedAmounts(address(this));
    }
    ...
    function convertToUnderlyingShares(
        ...
        return
            supply == 0
                ? shares
                : shares.mulDiv(
                IRewardTracker(router.stakedGlpTracker()).stakedAmounts(address(this)),
                supply,
                Math.Rounding.Up
            );
    }
